...
export function stripeReturnBase(origin: string | null, deploymentHost?: string) {
    ...
    const allowed = new Set([STRIPE_PREVIEW_HOST])
    if (deploymentHost && /^[a-z0-9-]+\.vercel\.app$/.test(deploymentHost)) allowed.add(deploymentHost)
    ...
    return `${url.origin}/admin/stripe`
}
