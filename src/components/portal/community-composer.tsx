...
export function CommunityComposer({ children }: { children: { id: string; full_name: string }[] }) {
    ...
    return <form ref={formRef} onSubmit={(event) => {
        ...
        startTransition(async () => {
            ...
            setMessage(result.success ? 'Enviado a Academy para revisión. Te avisaremos cuando se publique.' : result.error || 'No se pudo enviar.')
        ...
        <p className="mt-3 text-xs text-slate-500">Las publicaciones de familias se revisan antes de mostrarse a la comunidad. Máximo 20 MB.</p>
        {message && <p className="mt-3 text-sm font-medium text-navy" role="status">{message}</p>}
        <Button disabled={pending} className="mt-4 w-full bg-gold font-bold text-navy hover:bg-gold/80">{pending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Enviando…</> : <><Send className="mr-2 h-4 w-4" /> Enviar para revisión</>}</Button>
    </form>
}
