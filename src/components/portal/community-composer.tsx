...
export function CommunityComposer({ children }: { children: { id: string; full_name: string; can_share_media: boolean }[] }) {
    ...
    return <form ref={formRef} onSubmit={(event) => {
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
            <select name="child_id" required className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm"><option value="">Jugador</option>{children.map((child) => <option key={child.id} value={child.id}>{child.full_name}{child.can_share_media ? '' : ' · solo texto'}</option>)}</select>
        ...
        <p className="mt-3 flex gap-2 text-xs text-slate-500"><ShieldCheck className="h-4 w-4 shrink-0 text-gold" />Comparte solo contenido deportivo apropiado. Para adjuntar foto o vídeo, el jugador debe tener autorizado el uso interno en Autorizaciones. Academy puede retirar publicaciones que no respeten la comunidad. Máximo 20 MB.</p>
