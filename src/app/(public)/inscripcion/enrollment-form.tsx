...
export default function EnrollmentForm({
    ...
    if (state.success) {
        return <section className="mx-auto mt-10 max-w-2xl rounded-3xl border border-green-200 bg-white p-10 text-center shadow-lg" role="status"><CheckCircle2 className="mx-auto h-14 w-14 text-green-600" /><h2 className="mt-5 font-heading text-3xl font-black text-navy">Solicitud recibida</h2><p className="mt-3 text-slate-600">{state.message}</p><Button asChild className="mt-7 bg-gold font-bold text-navy hover:bg-gold/80"><Link href="/">Volver a la web</Link></Button></section>
    }

    return (
        <form action={action} className="mx-auto mt-10 max-w-3xl space-y-8 rounded-3xl bg-white p-6 shadow-xl md:p-10">
            <section>
                <h2 className="font-heading text-2xl font-black text-navy">1. ¿Qué quieres solicitar?</h2>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    {(Object.keys(serviceLabels) as EnrollmentService[]).map((item) => <label key={item} className={`cursor-pointer rounded-xl border p-4 transition ${service === item ? 'border-gold bg-gold/10 ring-1 ring-gold' : 'border-slate-200 hover:border-gold/60'}`}><input className="sr-only" type="radio" name="service" value={item} checked={service === item} onChange={() => { setService(item); setActivityId('') }} /><span className="block font-bold text-navy">{serviceLabels[item]}</span><span className="mt-1 block text-xs text-slate-500">{item === 'academy' ? 'Temporada deportiva' : item === 'campus' ? 'Edición intensiva' : 'Competición o evento'}</span></label>)}
                </div>
            ...
            <Button disabled={pending || (service !== 'academy' && activities.length === 0)} className="h-12 w-full bg-gold text-base font-black text-navy hover:bg-gold/80">{pending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Enviando solicitud…</> : <><Send className="mr-2 h-4 w-4" /> Enviar solicitud de inscripción</>}</Button>
