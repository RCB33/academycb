...
export default async function CampusPage() {
    ...
    return (
        <div className="min-h-screen bg-navy text-white">
            ...
            <section className="container py-20"><div className="rounded-[2rem] border border-gold/25 bg-gradient-to-br from-gold/10 to-white/[.03] p-7 md:p-10"><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-gold">Nuestros campus en números</p><h2 className="mt-2 font-heading text-3xl font-black uppercase text-white md:text-5xl">Una historia que<br />seguimos construyendo.</h2></div><p className="max-w-sm text-sm leading-relaxed text-slate-400">Mostraremos aquí únicamente cifras reales y actualizables de cada edición.</p></div><div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4">{['Jugadores participantes', 'Ediciones realizadas', 'Sedes simultáneas', 'Campus durante el año'].map((label) => <div key={label} className="rounded-2xl border border-white/10 bg-navy/50 p-5"><p className="font-heading text-4xl font-black text-gold">—</p><p className="mt-2 text-xs font-bold uppercase tracking-wide text-slate-300">{label}</p></div>)}</div></div></section>
