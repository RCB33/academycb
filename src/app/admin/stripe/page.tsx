...
export default async function StripeTestPage({ searchParams }: { searchParams: Promise<{ session_id?: string; cancelled?: string }> }) {
    ...
    const { data: attempts, error: ledgerError } = await supabase.from('stripe_test_attempts').select('id,state,created_at,confirmed_at').eq('owner_id', user.id).order('created_at', { ascending: false }).limit(10)
    ...
    return <div className="mx-auto max-w-3xl space-y-5">
        ...
        <section className="space-y-4 rounded-2xl border bg-white p-6">
            ...
            <p>{webhookConfigured ? 'Secreto configurado. Falta comprobar la entrega de un evento real de pruebas.' : 'Pendiente: configurar STRIPE_WEBHOOK_SECRET en Preview y el destino en Stripe.'}</p>
